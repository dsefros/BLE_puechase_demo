...
class AdvertisementPacketParser(
    ...
    fun parse(serviceData: ByteArray): Result<AdvertisementPacket> = runCatching {
        Log.d("BLE_Parser", "=== Parsing serviceData ===")
        Log.d("BLE_Parser", "ServiceData size: ${serviceData.size}")
        Log.d("BLE_Parser", "ServiceData hex: ${serviceData.joinToString("") { "%02x".format(it) }}")
        ...
        Log.d("BLE_Parser", "Packet version: $packetVersion, expected: ${VolnaContract.supportedPacketVersion}")
        ...
        Log.d("BLE_Parser", "RSSI delta: $rssiDelta")

        val terminalCapabilities = serviceData[1].toInt() and 0xFF
        Log.d("BLE_Parser", "Terminal capabilities: 0x${terminalCapabilities.toString(16)}")
        require((terminalCapabilities and VolnaContract.requiredOnlineC2bCapabilityMask) != 0) {
            "Online C2B capability not supported: 0x${terminalCapabilities.toString(16)}"
        }

        val operationCounter = serviceData[2].toInt() and 0xFF
        Log.d("BLE_Parser", "Operation counter: $operationCounter")
        ...
        Log.d("BLE_Parser", "QR ID bytes size: ${qrcIdBytes.size}")

        val qrcId = qrcIdConverter.fromBinary(qrcIdBytes)
        Log.d("BLE_Parser", "QR ID: $qrcId")
        Log.d("BLE_Parser", "=== Parse SUCCESS ===")
